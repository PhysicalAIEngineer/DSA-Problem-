...
public:
    ...
    int minimumPairRemoval(vector<int>& nums) {
        ...
        // copy nums into temp
        // temp will store the current value of each element
        ...
        // store all adjacent pairs as:
        // (sum of pair, index of first element)
        ...
        //
        // these arrays help us simulate deleting/merging
        // elements without actually shifting the array
        ...
        // count adjacent pairs that violate
        // the non-decreasing order
        //
        // example: [1, 5, 3]
        // pair (5, 3) is bad because 5 > 3
        int badPairs = 0;
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            // this pair is not in sorted order
            if (temp[i] > temp[i + 1]) {
                badPairs++;
            }
        ...
        // continue until there are no bad adjacent pairs
        // i.e. the array becomes non-decreasing
        while (badPairs > 0) {

            // get the pair having the minimum sum
            //
            // set stores pairs in sorted order,
            // so begin() gives the smallest pair
            ...
            // if the selected pair itself was a bad pair,
            // removing it decreases the bad pair count
            if (temp[first] > temp[second]) {
                badPairs--;
            }

            // check the pair on the left side
            // before: {d, (a, b)}
            // after merging: {d, (a+b)}
            if (first_left >= 0) {
            ...
            // check the pair on the right side
            // before: {(a, b), d}
            // after merging: {(a+b), d}
            if (second_right < n) {
            ...
            if (second_right < n) {
                ...
                minPairSet.erase({
                    temp[second] + temp[second_right],
                    second
                });

                // add the new right pair
                // the merged element is stored at index 'first'
                minPairSet.insert({
                    temp[first] + temp[second] + temp[second_right],
                    first
                });

                // previous active element of second_right
                // is now 'first'
                prevIndex[second_right] = first;
            }

            // next active element after 'first'
            // is now whatever came after 'second'
            ...
            // first = a
            // second = b
            // a + b becomes the new value at first
