...
class Solution:
    def findKthBit(self, n: int, k: int) -> str:
        # base case : S1 = "0" so the only characters is "0"
        if n == 1:
            return "0"
        # calculate the length of the current string Sn Formula: Sn = 2^n - 1
        length = (1 << n) - 1
        # find the middle position of Sn the middle characters is always "1"
        mid = (length + 1) // 2
        # if the k-th position lies in the left half recursively search in S(n-1)
        if k < mid:
            return self.findKthBit(n - 1, k)
        # if the k-th position is exactly the middle return the fixed middle characters
        elif k == mid:
            return "1"
        # otherwise the k-th position lies in the right half
            ...
            # recursively find the the bit at the mirrored position
